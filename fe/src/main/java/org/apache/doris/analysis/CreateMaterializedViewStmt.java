...
public class CreateMaterializedViewStmt extends DdlStmt {
    ...
    static {
        fnNameToPatten = Maps.newHashMap();
        fnNameToPatten.put(AggregateType.SUM.name().toLowerCase(),
                new MVColumnOneChildPatten(AggregateType.SUM.name().toLowerCase()));
        fnNameToPatten.put(AggregateType.MIN.name().toLowerCase(),
                new MVColumnOneChildPatten(AggregateType.MIN.name().toLowerCase()));
        fnNameToPatten.put(AggregateType.MAX.name().toLowerCase(),
                new MVColumnOneChildPatten(AggregateType.MAX.name().toLowerCase()));
        fnNameToPatten.put("count", new MVColumnOneChildPatten("count"));
        fnNameToPatten.put("bitmap_union", new MVColumnBitmapUnionPatten());
        fnNameToPatten.put("hll_union", new MVColumnHLLUnionPatten());
    }
    ...
    public void analyzeSelectClause() throws AnalysisException {
        ...
        for (int i = 0; i < selectList.getItems().size(); i++) {
            ...
            if (selectListItem.getExpr() instanceof SlotRef) {
                ...
                // Function must match patten.
                FunctionCallExpr functionCallExpr = (FunctionCallExpr) selectListItem.getExpr();
                String functionName = functionCallExpr.getFnName().getFunction();
                MVColumnPatten mvColumnPatten = fnNameToPatten.get(functionName.toLowerCase());
                if (mvColumnPatten == null) {
                    throw new AnalysisException(
                            "Materialized view does not support this function:" + functionCallExpr.toSqlImpl());
                }
                if (!mvColumnPatten.match(functionCallExpr)) {
                    throw new AnalysisException(
                            "The function " + functionName + " must match patten:" + mvColumnPatten.toString());
                }
                // check duplicate column
