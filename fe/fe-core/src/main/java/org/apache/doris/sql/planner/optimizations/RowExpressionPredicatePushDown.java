...
import org.apache.doris.sql.expressions.RowExpressionNodeInliner;
...
import org.apache.doris.sql.planner.RowExpressionPredicateExtractor;
...
import static org.apache.doris.sql.relational.Expressions.uniqueSubExpressions;
        ...
        implements PlanOptimizer
{
    ...
    public RowExpressionPredicatePushDown(Metadata metadata, SqlParser sqlParser)
    {
        ...
        this.effectivePredicateExtractor = new RowExpressionPredicateExtractor(metadata.getFunctionManager(), metadata.getTypeManager());
            ...
            extends SimplePlanRewriter<RowExpression>
    {
        ...
        {
            LogicalPlanNode rewrittenNode = context.defaultRewrite(node, TRUE_CONSTANT);
        ...
        @Override
        public LogicalPlanNode visitProject(ProjectNode node, RewriteContext<RowExpression> context)
        {
            ...
            LogicalPlanNode rewrittenNode = context.defaultRewrite(node, logicalRowExpressions.combineConjuncts(inlinedDeterministicConjuncts));
        ...
        @Override
        public LogicalPlanNode visitGroupId(GroupIdNode node, RewriteContext<RowExpression> context)
        {
            ...
            LogicalPlanNode rewrittenNode = context.defaultRewrite(node, RowExpressionVariableInliner.inlineVariables(commonGroupingVariableMapping, logicalRowExpressions.combineConjuncts(conjuncts.get(true))));
        ...
        @Override
        public LogicalPlanNode visitSort(SortNode node, RewriteContext<RowExpression> context)
        {
        ...
        @Deprecated
        @Override
        public LogicalPlanNode visitFilter(FilterNode node, RewriteContext<RowExpression> context)
        {
            LogicalPlanNode rewrittenPlan = context.rewrite(node.getSource(), logicalRowExpressions.combineConjuncts(node.getPredicate(), context.get()));
        ...
        @Override
        public LogicalPlanNode visitJoin(JoinNode node, RewriteContext<RowExpression> context)
        {
        ...
        private boolean isOperation(RowExpression expression, OperatorType type) {
            if (expression instanceof CallExpression) {
                ...
                if (type.equals(OperatorType.EQUAL)) {
                    if(call.getFunctionHandle().getFunctionName().equalsIgnoreCase("eq")) {
                        return true;
                    } else {
                        return false;
                    }
        ...
        @Override
        public LogicalPlanNode visitSemiJoin(SemiJoinNode node, RewriteContext<RowExpression> context)
        {
            RowExpression inheritedPredicate = context.get();
        ...
        private LogicalPlanNode visitNonFilteringSemiJoin(SemiJoinNode node, RewriteContext<RowExpression> context)
        {
            RowExpression inheritedPredicate = context.get();
            ...
            LogicalPlanNode rewrittenFilteringSource = context.defaultRewrite(node.getFilteringSource(), TRUE_CONSTANT);
            ...
            LogicalPlanNode rewrittenSource = context.rewrite(node.getSource(), logicalRowExpressions.combineConjuncts(sourceConjuncts));
        ...
        private LogicalPlanNode visitFilteringSemiJoin(SemiJoinNode node, RewriteContext<RowExpression> context)
        {
            RowExpression inheritedPredicate = context.get();
            ...
            LogicalPlanNode rewrittenSource = context.rewrite(node.getSource(), logicalRowExpressions.combineConjuncts(sourceConjuncts));
            LogicalPlanNode rewrittenFilteringSource = context.rewrite(node.getFilteringSource(), logicalRowExpressions.combineConjuncts(filteringSourceConjuncts));
        ...
        @Override
        public LogicalPlanNode visitAggregation(AggregationNode node, RewriteContext<RowExpression> context)
        {
            ...
            LogicalPlanNode rewrittenSource = context.rewrite(node.getSource(), logicalRowExpressions.combineConjuncts(pushdownConjuncts));
        ...
        @Override
        public LogicalPlanNode visitTableScan(TableScanNode node, RewriteContext<RowExpression> context)
        {
