...
import org.apache.doris.sql.planner.EffectivePredicateExtractor;
import org.apache.doris.sql.planner.EqualityInference;
            ...
            extends SimplePlanRewriter<Expression>
    {
        ...
        public LogicalPlanNode visitProject(ProjectNode node, RewriteContext<Expression> context)
        {
            Set<VariableReferenceExpression> deterministicVariables = node.getAssignments().entrySet().stream()
                    .filter(entry -> isDeterministic(castToExpression(entry.getValue())))
        ...
        @Override
        public LogicalPlanNode visitGroupId(GroupIdNode node, RewriteContext<Expression> context)
        {
            ...
            LogicalPlanNode rewrittenNode = context.defaultRewrite(node, inlineVariables(commonGroupingVariableMapping, combineConjuncts(conjuncts.get(true)), types));
        ...
        @Override
        public LogicalPlanNode visitMarkDistinct(MarkDistinctNode node, RewriteContext<Expression> context)
        {
            ...
            LogicalPlanNode rewrittenNode = context.defaultRewrite(node, combineConjuncts(conjuncts.get(true)));
        ...
        @Override
        public LogicalPlanNode visitAssignUniqueId(AssignUniqueId node, RewriteContext<Expression> context)
        {
