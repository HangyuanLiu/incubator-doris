...
import org.apache.doris.sql.planner.optimizations.JoinGraph;
        ...
        implements Rule<JoinNode>
{
    ...
    public Result apply(JoinNode node, Captures captures, Context context)
    {
        ...
        LogicalPlanNode replacement = buildJoinTree(node.getOutputVariables(), joinGraph, joinOrder, context.getIdAllocator());
    ...
    {
        ImmutableList.Builder<LogicalPlanNode> joinOrder = ImmutableList.builder();
        ...
        PriorityQueue<LogicalPlanNode> nodesToVisit = new PriorityQueue<>(
                graph.size(),
                comparing(node -> priorities.get(node.getId())));
        Set<LogicalPlanNode> visited = new HashSet<>();

        nodesToVisit.add(graph.getNode(0));

        while (!nodesToVisit.isEmpty()) {
            LogicalPlanNode node = nodesToVisit.poll();
            ...
            if (nodesToVisit.isEmpty() && visited.size() < graph.size()) {
                ...
                Optional<LogicalPlanNode> firstNotVisitedNode = graph.getNodes().stream()
    ...
    public static LogicalPlanNode buildJoinTree(List<VariableReferenceExpression> expectedOutputVariables, JoinGraph graph, List<Integer> joinOrder, PlanNodeIdAllocator idAllocator)
    {
