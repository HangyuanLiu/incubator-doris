...
import static org.apache.doris.sql.planner.cost.CostCalculatorWithEstimatedExchanges.calculateLocalRepartitionCost;
import static org.apache.doris.sql.planner.cost.CostCalculatorWithEstimatedExchanges.calculateRemoteGatherCost;
import static org.apache.doris.sql.planner.cost.CostCalculatorWithEstimatedExchanges.calculateRemoteRepartitionCost;
import static org.apache.doris.sql.planner.cost.CostCalculatorWithEstimatedExchanges.calculateRemoteReplicateCost;
import static org.apache.doris.sql.planner.cost.LocalCostEstimate.addPartialComponents;
        ...
        implements CostCalculator
{
    ...
    @Override
    public PlanCostEstimate calculateCost(LogicalPlanNode node, StatsProvider stats, CostProvider sourcesCosts, Session session)
    {
            ...
            extends PlanVisitor<PlanCostEstimate, Void>
    {
        ...
        @Override
        public PlanCostEstimate visitPlan(LogicalPlanNode node, Void context)
        {
        ...
        private LocalCostEstimate calculateJoinCost(LogicalPlanNode join, LogicalPlanNode probe, LogicalPlanNode build, boolean replicated)
        {
        ...
        private LocalCostEstimate calculateJoinOutputCost(LogicalPlanNode join)
        {
        ...
        private PlanCostEstimate costForSource(LogicalPlanNode node, LocalCostEstimate localCost)
        {
        ...
        private PlanCostEstimate costForAccumulation(LogicalPlanNode node, LocalCostEstimate localCost)
        {
        ...
        private PlanCostEstimate costForStreaming(LogicalPlanNode node, LocalCostEstimate localCost)
        {
            PlanCostEstimate sourcesCost = getSourcesEstimations(node)
                    .reduce(PlanCostEstimate.zero(), CostCalculatorUsingExchanges::addParallelSiblingsCost);
            return new PlanCostEstimate(
        ...
        private PlanNodeStatsEstimate getStats(LogicalPlanNode node)
        {
            return stats.getStats(node);
        }

        private Stream<PlanCostEstimate> getSourcesEstimations(LogicalPlanNode node)
        {
