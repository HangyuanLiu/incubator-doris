...
public class StatsAndCosts
{
    ...
    public StatsAndCosts getForSubplan(LogicalPlanNode root)
    {
        Iterable<LogicalPlanNode> planIterator = Traverser.forTree(LogicalPlanNode::getSources)
        ...
        for (LogicalPlanNode node : planIterator) {
    ...
    public static StatsAndCosts create(LogicalPlanNode root, StatsProvider statsProvider, CostProvider costProvider)
    {
        Iterable<LogicalPlanNode> planIterator = Traverser.forTree(LogicalPlanNode::getSources)
                .depthFirstPreOrder(root);
